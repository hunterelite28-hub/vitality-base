...
`brand.html`, `peak.html`, `train.html`, `vee.html`, `vitals.html` are currently byte-identical to their `tiles-library` counterparts, so the same line numbers and edits from Task 7 apply verbatim here. `finance.html` has diverged (extra `--alert`/`--alert-ink` fields not in `tiles-library`) — apply the substitution independently using this file's own line numbers below.

- [ ] **Step 1: `public/tiles/brand.html`** — same edits as Task 7 Step 1.

- [ ] **Step 2: `public/tiles/peak.html`** — same edits as Task 7 Step 4.

- [ ] **Step 3: `public/tiles/train.html`** — same edits as Task 7 Step 5.

- [ ] **Step 4: `public/tiles/vee.html`** — same edits as Task 7 Step 6.

- [ ] **Step 5: `public/tiles/vitals.html`** — same edits as Task 7 Step 7.

- [ ] **Step 6: `public/tiles/finance.html`** (own line numbers — diverged from `tiles-library`)
...
- [ ] **Step 7: Confirm `public/tiles/fuel.html` needs no change**
